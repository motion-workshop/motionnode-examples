...
You can set a static IP address on a MotionNode POE device using the included [set_static_ip.py](./scripts/set_static_ip.py) script. 
...
The following example command will search for the connected MotionNode POE device on the network using Zeroconf, and set a new static ip of **192.168.1.100**.
...
If the device is not found (Zeroconf fails), you can specify the device's current ip address using the *--host* parameter.  For example, if the current MotionNode IP address is 192.168.1.15:
...
You can also remove a static ip address and revert to DHCP.  For example, if the current ip address is 192.168.1.15:

```
python set_static_ip.py --host 192.168.1.15 --ip "" --gateway "" --dns ""
```
